		...
		$query = "
			SELECT 
				...
				CASE 
					...
					THEN ROUND((report.TOTAL_LAHAN / report.PLANT_AREA) * 100, 2) 
					ELSE 0
				END AS PLANT_AREA_ESTIMATE,
				CASE 
					WHEN report.TOTAL > 0 AND report.TOTAL_30HARI > 0 
					THEN ROUND((report.TOTAL_30HARI / report.TOTAL) * 100, 2) 
					ELSE 0
				END AS day30,
				CASE 
					WHEN report.TOTAL > 0 AND report.TOTAL_60HARI > 0 
					THEN ROUND((report.TOTAL_60HARI / report.TOTAL) * 100, 2) 
					ELSE 0
				END AS day60,
				CASE 
					WHEN report.TOTAL > 0 AND report.TOTAL_90HARI > 0 
					THEN ROUND((report.TOTAL_90HARI / report.TOTAL) * 100, 2) 
					ELSE 0
				END AS day90,
				CASE 
					WHEN report.TOTAL > 0 AND report.TOTAL_90UPHARI > 0 
					THEN ROUND((report.TOTAL_90UPHARI / report.TOTAL) * 100, 2) 
					ELSE 0
				END AS day90plus,
				report.ESTIMASI1,
				report.ESTIMASI2,
			...
			FROM (
				SELECT 
					...
					A.PLANT_AREA, 
					COALESCE(
						(SELECT SUM(LUAS_LAHAN) 
						FROM SURVEY D 
						WHERE D.DISTRICT = A.ID_DISTRICTS 
					...
					(COALESCE(
						...
						FROM SURVEY D 
						WHERE D.DISTRICT = A.ID_DISTRICTS 
						GROUP BY D.DISTRICT), 0) * 5.5
					) AS TOTAL_PRODUKSI,
					COALESCE(
						(SELECT COUNT(UMUR_TANAM) 
						FROM SURVEY D
						WHERE CURRENT_PHASE IS NOT NULL 
						AND (UMUR_TANAM BETWEEN 0 AND 30)  
					...
					COALESCE(
						...
						FROM SURVEY D
						WHERE CURRENT_PHASE IS NOT NULL 
						AND (UMUR_TANAM BETWEEN 31 AND 60)  
						AND D.DISTRICT = A.ID_DISTRICTS 
						GROUP BY D.DISTRICT), 0
					) AS TOTAL_60HARI,
					COALESCE(
						...
						FROM SURVEY D
						WHERE CURRENT_PHASE IS NOT NULL 
						AND (UMUR_TANAM BETWEEN 61 AND 90)  
						AND D.DISTRICT = A.ID_DISTRICTS 
						GROUP BY D.DISTRICT), 0
					) AS TOTAL_90HARI,
					COALESCE(
